...
export const CardHeader: React.FC<{
  ...
  <div className={cn('flex flex-wrap items-start justify-between gap-3', className)}>
    <div className="min-w-0">
      <h2 className="text-sm font-black tracking-tight text-ink-900">{title}</h2>
  ...
  return (
    <Card padding="md" className="flex items-start justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className="mt-1.5 truncate text-2xl font-black tracking-tight text-ink-900">{value}</p>
        {hint && <p className="mt-1 text-[11px] font-semibold text-ink-500">{hint}</p>}
      </div>
      {icon && (
        <div
